...
binary_path="$binary_dir/agent-usage-menu"
claude_capture_path="$binary_dir/claude-usage-capture"
claude_statusline_path="$binary_dir/claude-usage-statusline"
agent_path="$agent_dir/com.agentusagemenu.app.plist"
legacy_agent_path="$agent_dir/com.codexusagemenu.app.plist"
enable_claude=false
...
install -m 755 "$project_dir/.build/release/AgentUsageMenu" "$binary_path"
...
install -m 644 "$project_dir/launchd/com.agentusagemenu.app.plist" "$agent_path"
...
rm -f "$legacy_agent_path" "$binary_dir/codex-usage-menu"
launchctl bootout "gui/$(id -u)/com.agentusagemenu.app" 2>/dev/null || true
sleep 1
launchctl bootstrap "gui/$(id -u)" "$agent_path"
if [[ "${claude_enabled:-false}" == true ]]; then
  echo "Installed. Agent Usage Menu is running with Codex and Claude Code support."
elif [[ "$enable_claude" == true ]]; then
  echo "Installed. Agent Usage Menu is running; Claude Code capture awaits status-line integration."
else
  echo "Installed. Agent Usage Menu is running. Run ./scripts/install.sh --claude to add Claude Code capture."
fi
